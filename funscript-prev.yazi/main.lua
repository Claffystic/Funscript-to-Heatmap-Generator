-- ~/.config/yazi/plugins/funscript-prev.yazi/main.lua

local M = {}

-- Peek at a file preview
function M:peek(job)
...
-- Preload preview data
function M:preload(job)
	ya.dbg("Preloading funscript-preview ***")
	if not job then
		return 1
	end
	...
	ya.err("Calling funscript2png for: " .. tostring(job.file.url) .. ", cache: " .. tostring(cache))
	local child, code = Command(os.getenv("HOME") .. "/.local/bin/funscript2png")
		:arg({ tostring(job.file.url), tostring(cache) })
		:spawn()

	if not child then
		ya.err("spawn `funscript2png` command returns " .. tostring(code))
		return 0
	end
